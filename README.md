...
- ランキングの種類を切り替えられる：**急上昇順 / いま伸びてる順 / 再生数順 / 高評価率順 / コメント数順 / 新着順**
...
**無料枠は 1 日 10,000 ユニット**です。このサイトの 1 回の収集は
`videos.list` が 15 回（カテゴリ数）＋ `channels.list` が数回で、**20 ユニット程度**しか使いません。
1 日 4 回動かしても 100 ユニット未満なので、枠の心配はまずありません。
...
3. `channels.list` で登録者数とチャンネルアイコンを足す
4. チャンネル別のランクイン本数を数えてチャンネルランキングを作る
5. すべてを `docs/data/rankings.json` に書き出し、フロントはこの 1 ファイルだけを読む
